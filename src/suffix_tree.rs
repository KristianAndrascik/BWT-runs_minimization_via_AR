...
impl<'s> SuffixTree<'s> {
    ...
    fn init<S>(s: S) -> SuffixTree<'s> where S: Into<Cow<'s, str>> {
        ...
        SuffixTree {
            text: s,
            root: Node::leaf(0,len as u32, 0, 0),
        }
    }
...
impl Node {
    fn leaf(id: usize, sufstart: u32, start: u32, end: u32) -> Box<Node> {
        Box::new(Node {
            id: id,
            parent: Rawlink::none(),
    ...
    fn internal(id: usize,start: u32, end: u32) -> Box<Node> {
        Box::new(Node {
            id: id,
            parent: Rawlink::none(),
            children: BTreeMap::new(),
    ...
    for (i, &sufstart) in sa.table().iter().enumerate() {
        ...
        if dv == lcp_len {
            ...
            let mut node = Node::leaf(
                id,sufstart, sufstart + lcp_len, sa.text().len() as u32);
            ...
            let mut int_node = Node::internal(id,table[i-1] + dv,
                                              table[i-1] + lcp_len);
            id += 1;
